...
export const EmailAssistant: React.FC<EmailAssistantProps> = ({ profile, onViewResume }) => {
  ...
  const scanInbox = useAction(api.emails.scanInbox);
  ...
  const handleScanInbox = async () => {
    ...
    try {
      ...
      setInboxEmails(emails as EmailMessage[]);
    } catch (e) {
      console.error(e);
    } finally {
      setScanning(false);
  ...
  const handleAnalyzeEmail = (email: EmailMessage) => {
    setIncomingText(email.fullBody);
            ...
            {inboxEmails.length === 0 && !scanning ? (
              <div className="h-full flex flex-col items-center justify-center text-center p-6">
                ...
                <p className="text-sm text-slate-500 mb-6 max-w-xs">
                  JobSense will scan your inbox for recruiter emails, job offers, and interview requests.
                </p>
                <button
              ...
              <div className="h-full flex flex-col items-center justify-center space-y-4">
                ...
                <p className="text-sm text-slate-500 font-medium">Analyzing emails for job opportunities...</p>
              </div>
            ) : (
              <div className="space-y-3">
                <div className="flex justify-between items-center mb-2 px-1">
                  <span className="text-xs font-bold text-slate-500 uppercase tracking-wider">Found {inboxEmails.length} relevant emails</span>
                  <button onClick={handleScanInbox} className="text-xs text-indigo-600 hover:text-indigo-800 flex items-center">
                ...
                {inboxEmails.map(email => (
                  ...
                  >
                    <div className={`absolute top-0 left-0 w-1 h-full ${email.priority === 'High' ? 'bg-indigo-500' : 'bg-slate-300'}`}></div>
                    <div className="pl-3">
                      <div className="flex justify-between items-start mb-1">
                        <h4 className="font-semibold text-slate-900 text-sm truncate pr-2">{email.sender}</h4>
